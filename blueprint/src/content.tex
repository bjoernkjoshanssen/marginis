...
\end{corollary}
\begin{proof}
By \Cref{Hippo}, $Y$ computes the limit of a subsequence $\{\Y_{N(b)}\}_{b\in\omega}$. By \Cref{satisfy_LLN}, this limit must be $p$.
\end{proof}
...
\begin{corollary}\label{noUniv}
	\uses{Hippo_compute, steveagrees}
There is no universal Hippocratic $\mu_p$-test, unless $p$ is computable. 
\end{corollary}
\begin{proof}
If there is such a test then by \Cref{Hippo_compute} there is a test $\{U_n\}_{n\in\omega}$ as in the hypothesis of \Cref{steveagrees}, whence $p$ is computable.
\end{proof}
