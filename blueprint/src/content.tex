...
\begin{theorem}\label{logicbike} %In fact, if $Y$ is Schnorr random with respect to the measure $\mu_p$ then $Y\ge_T p$.
...
\]
Then $\{V_d\}_{d\in\omega}$ is uniformly $\Sigma^0_1$. Recall from \Cref{one} that we defined
\[
...
|\Y_{N(b+1)}-q| <  2^{-b};
\]
if we define $\Theta_d$ as $\Psi_d$ in \Cref{logicbike} except with $N(\cdot)$ replaced by $N(\cdot+1)$, then
\[
q_n = \Theta_d^Y(n).
\] 
